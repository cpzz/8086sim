...
function updateInstructionsDisplay() {
    ...
    // 创建数据定义地址集合，用于区分代码标签和数据定义标签
    const dataDefinitionAddresses = new Set();
    
    // 收集数据段定义的地址
    if (assembler.dataSegments) {
        assembler.dataSegments.forEach(dataSeg => {
            if (dataSeg.label && dataSeg.label.trim() !== '') {
                dataDefinitionAddresses.add(dataSeg.offset);
            }
        });
    }
    ...
    if (assembler.codeDataSegments) {
        assembler.codeDataSegments.forEach(codeDataSeg => {
            if (codeDataSeg.label && codeDataSeg.label.trim() !== '') {
                dataDefinitionAddresses.add(codeDataSeg.offset);
            }
        });
    }
    
    // 特别处理：收集所有指令地址，这些都不是数据定义
    instructions.forEach(instruction => {
        // 指令地址不应该被视为数据定义地址
        // 这样做是为了确保代码段中的标签（如过程定义）不会被误认为是数据
    });

    // 创建一个映射，将地址映射到指令对象，方便快速查找
    ...
    for (const label in assembler.symbols) {
        ...
        if (typeof symbolAddr === 'number' && 
            ...
            symbolAddr >= 0 && 
            symbolAddr <= 0xFFFF && 
            !dataDefinitionAddresses.has(symbolAddr)) { // 排除数据定义的地址
            codeLabels.push({ label, address: symbolAddr });
        }
    }
    // 按地址排序
    codeLabels.sort((a, b) => a.address - b.address);

    // 遍历所有指令，同时处理标签显示
    let currentLabelIndex = 0; // 指向下一个要处理的标签
    instructions.forEach(instruction => {
        // 显示所有地址小于等于当前指令地址的标签
        while (currentLabelIndex < codeLabels.length && 
               codeLabels[currentLabelIndex].address <= instruction.address) {
            const labelInfo = codeLabels[currentLabelIndex];
            
            // 创建标签行
            const labelRowElement = document.createElement('div');
            labelRowElement.className = 'instructions-table-row label-row';

            // 创建地址列（显示标签的地址值）
            const labelAddressCell = document.createElement('div');
            labelAddressCell.className = 'instructions-table-cell address label-address';
            const labelAddrStr = 'CS:' + labelInfo.address.toString(16).toUpperCase().padStart(5, '0');
            labelAddressCell.textContent = labelAddrStr;
            labelRowElement.appendChild(labelAddressCell);

            // 创建机器代码列（标签行为空）
            const labelMachineCodeCell = document.createElement('div');
            labelMachineCodeCell.className = 'instructions-table-cell machine-code';
            labelMachineCodeCell.textContent = '';
            labelRowElement.appendChild(labelMachineCodeCell);

            // 创建汇编代码列（标签行显示标签名称）
            const labelAssemblyCell = document.createElement('div');
            labelAssemblyCell.className = 'instructions-table-cell assembly label-assembly';
            labelAssemblyCell.textContent = labelInfo.label + ':';
            labelAssemblyCell.style.fontWeight = 'bold';
            labelAssemblyCell.style.color = '#0066cc';
            labelRowElement.appendChild(labelAssemblyCell);

            // 创建注释列（标签行为空）
            const labelCommentCell = document.createElement('div');
            labelCommentCell.className = 'instructions-table-cell comment';
            labelCommentCell.textContent = '';
            labelRowElement.appendChild(labelCommentCell);

            instructionsList.appendChild(labelRowElement);
            
            currentLabelIndex++; // 移动到下一个标签
        }
        ...
        // 构建汇编代码显示（包括操作码和操作数）
        let assemblyCode = instruction.opcode || '';
        if (instruction.operands && instruction.operands.length > 0) {
            // 尝试使用原始大小写的标签名
            const processedOperands = instruction.operands.map(op => {
